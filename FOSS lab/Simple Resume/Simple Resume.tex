...
\item Client interactions / communication on project ongoing status
\item Implementing reusable components for future similar projects.
\item Creating, modifying and debugging TIBCO BW engines, monitoring the same in ADMIN.
\item Designing Workflow and Business Works processes using TIBCO Staff ware EAI PJAVA plug-in.
\item Designed BW interfaces to automate the process of clearing the orders for recurring issues.
\item Identifying potential problems due to CR rolled outs in pre-rollout reviews and suggesting possible solution to mitigate the risk involved with rollouts.
\item Delegating responsibilities to various team members and monitor the progress in each of them to ensure ontime delivery.
\item Participated in con-calls with different business units and provided resolutions to the issues reported during the calls.
\item Analyze and identify the procedural activities which have a scope of automation and design scripts for automation.
\end{itemize} \\
\hline
\end{tabular}
}
